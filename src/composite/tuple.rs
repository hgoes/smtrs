...
#[derive(Clone)]
pub struct Element1Of3;
#[derive(Clone)]
pub struct Element2Of3;
#[derive(Clone)]
pub struct Element3Of3;
...
impl<'a,A: 'a,B: 'a,C: 'a> SimplePathEl<'a,(A,B,C)> for Element1Of3 {
    type To = A;
    fn get<'b>(&self,from: &'b (A,B,C)) -> &'b Self::To where 'a: 'b {
        &from.0
    }
    fn get_mut<'b>(&self,from: &'b mut (A,B,C)) -> &'b mut Self::To where 'a: 'b {
        &mut from.0
    }
}

impl<'a,Em: Embed,A: 'a,B: 'a,C: 'a> PathEl<'a,Em,(A,B,C)> for Element1Of3 {
    fn read<PrevFrom,Prev: Path<'a,Em,PrevFrom,To=(A,B,C)>>(
        &self,
        prev: &Prev,from: &PrevFrom,
        pos: usize,src: &[Em::Expr],em: &mut Em)
        -> Result<Em::Expr,Em::Error> {
    ...
    fn read_slice<'b,PrevFrom,Prev: Path<'a,Em,PrevFrom,To=(A,B,C)>>(
        &self,prev: &Prev,from: &PrevFrom,pos: usize,len: usize,src: &'b [Em::Expr])
        -> Option<&'b [Em::Expr]> {
        prev.read_slice(from,pos,len,src)
    }
    fn write<PrevFrom,Prev: Path<'a,Em,PrevFrom,To=(A,B,C)>>(
        &self,prev: &Prev,from: &PrevFrom,
        pos: usize,ne: Em::Expr,
        trg: &mut Vec<Em::Expr>,em: &mut Em)
        -> Result<(),Em::Error> {
    ...
    fn write_slice<PrevFrom,Prev: Path<'a,Em,PrevFrom,To=(A,B,C)>>(
        &self,prev: &Prev,from: &mut PrevFrom,pos: usize,old_len: usize,
        src: &mut Vec<Em::Expr>,trg: &mut Vec<Em::Expr>,em: &mut Em)
        -> Result<(),Em::Error> {
        prev.write_slice(from,pos,old_len,src,trg,em)
...
impl<'a,A: 'a+HasSorts,B: 'a,C: 'a> SimplePathEl<'a,(A,B,C)> for Element2Of3 {
    type To = B;
    fn get<'b>(&self,from: &'b (A,B,C)) -> &'b Self::To where 'a: 'b {
        &from.1
    }
...
impl<'a,Em: Embed,A: 'a+HasSorts,B: 'a,C: 'a> PathEl<'a,Em,(A,B,C)> for Element2Of3 {
    fn read<PrevFrom,Prev: Path<'a,Em,PrevFrom,To=(A,B,C)>>(
        &self,
        prev: &Prev,from: &PrevFrom,
        pos: usize,src: &[Em::Expr],em: &mut Em)
...
impl<'a,A: 'a+HasSorts,B: 'a+HasSorts,C: 'a> SimplePathEl<'a,(A,B,C)> for Element3Of3 {
    type To = C;
    fn get<'b>(&self,from: &'b (A,B,C)) -> &'b Self::To where 'a: 'b {
        &from.2
    }
...
impl<'a,Em: Embed,A: 'a+HasSorts,B: 'a+HasSorts,C: 'a> PathEl<'a,Em,(A,B,C)> for Element3Of3 {
    fn read<PrevFrom,Prev: Path<'a,Em,PrevFrom,To=(A,B,C)>>(
        &self,
        prev: &Prev,from: &PrevFrom,
        pos: usize,src: &[Em::Expr],em: &mut Em)
        -> Result<Em::Expr,Em::Error> {
        let len0 = prev.get(from).0.num_elem();
        let len1 = prev.get(from).1.num_elem();
        prev.read(from,len0+len1+pos,src,em)
    }
    fn read_slice<'b,PrevFrom,Prev: Path<'a,Em,PrevFrom,To=(A,B,C)>>(
        &self,prev: &Prev,from: &PrevFrom,pos: usize,len: usize,src: &'b [Em::Expr])
        -> Option<&'b [Em::Expr]> {
        let len0 = prev.get(from).0.num_elem();
        let len1 = prev.get(from).1.num_elem();
        prev.read_slice(from,len0+len1+pos,len,src)
    }
    fn write<PrevFrom,Prev: Path<'a,Em,PrevFrom,To=(A,B,C)>>(
        &self,prev: &Prev,from: &PrevFrom,
        pos: usize,ne: Em::Expr,
        ...
        let len0 = prev.get(from).0.num_elem();
        let len1 = prev.get(from).1.num_elem();
        prev.write(from,len0+len1+pos,ne,trg,em)
    }
    fn write_slice<PrevFrom,Prev: Path<'a,Em,PrevFrom,To=(A,B,C)>>(
        &self,prev: &Prev,from: &mut PrevFrom,pos: usize,old_len: usize,
        src: &mut Vec<Em::Expr>,trg: &mut Vec<Em::Expr>,em: &mut Em)
        ...
        let len0 = prev.get(from).0.num_elem();
        let len1 = prev.get(from).1.num_elem();
        prev.write_slice(from,len0+len1+pos,old_len,src,trg,em)
    }
}
...
impl<A: HasSorts,B: HasSorts,C: HasSorts> HasSorts for (A,B,C) {
    ...
    fn elem_sort<Em: Embed>(&self,n: usize,em: &mut Em)
        ...
        let len1 = self.1.num_elem();
        if n<len0+len1 {
            return self.1.elem_sort(n-len0,em)
        }
        self.2.elem_sort(n-len0-len1,em)
    }
}
...
impl<'a,
     A: Composite<'a>,
     B: Composite<'a>,
     C: Composite<'a>> Composite<'a> for (A,B,C) {
    fn combine<Em,FromL,PL,FromR,PR,FComb,FL,FR>(
        pl: &PL,froml: &FromL,srcl: &[Em::Expr],
        pr: &PR,fromr: &FromR,srcr: &[Em::Expr],
        comb: &FComb,only_l: &FL,only_r: &FR,
        ...
        if let Some(nel1) = A::combine(&el1_l,froml,srcl,
            ...
            if let Some(nel2) = B::combine(&el2_l,froml,srcl,
                                           ...
                                           comb,only_l,only_r,
                                           res,em)? {
                let el3_l = Then { first: pl.clone(),
                                   then: Element3Of3 };
                let el3_r = Then { first: pr.clone(),
                                   then: Element3Of3 };

                if let Some(nel3) = C::combine(&el3_l,froml,srcl,
                                               &el3_r,fromr,srcr,
                                               comb,only_l,only_r,
                                               res,em)? {
                    Ok(Some((nel1,nel2,nel3)))
                } else {
                    Ok(None)
                }
            } else {
    ...
    fn invariant<Em,From,P>(path: &P,from: &From,src: &[Em::Expr],
        ...
        let p2 = Then { first: path.clone(),
                        then: Element2Of3 };
        B::invariant(&p2,from,src,res,em)?;
        let p3 = Then { first: path.clone(),
                        then: Element3Of3 };
        C::invariant(&p3,from,src,res,em)
    }
}
